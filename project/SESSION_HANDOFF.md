...
Recipe/query *resolution* (turning a reference/query into the candidate(s) it currently designates), persistent matching, ambiguity selection, authoritative fingerprint fallback, raw-handle epochs, reference health reporting, Stage-4 source API, Stage-5 raw geometry, generalized feature tracing, interfaces, assemblies, configurations, external-asset infrastructure, and AICAD-101+ tasks remain unimplemented. `AICAD-082`..`084` added per-candidate predicate *evaluation* (given an already-identified candidate and, where needed, already-resolved evidence) — a real but bounded step short of all of the above; see `project/reports/AICAD-082.md`..`084.md`'s own "Limitations" sections for exactly which `TopologyPredicate`/`SpatialPredicate` variants remain unspecified pending a later task. `AICAD-085`..`087` added explicit-export registration and real per-operation lineage capture/classification (`unchanged`/`modified`/`split`/`deleted` for prior entities, `new`/`merged` for result entities) against a live build — but, like `082`..`084`, addressed only by live `cad_occt_bridge::Shape`, not yet by `FeatureAnchor`/persistent `AnyRef` identity or a real `FeatureGraph`/`ParametricBuildSession` build; see `project/reports/AICAD-085.md`..`087.md`'s own "Limitations" sections.
...
3. all sequential Stage-4 agents use the newest `origin/claude/aicad-stage4-dev` as canonical working state; **in force** — the branch exists and Batches S4-01/S4-02 synchronized to it.
4. do not independently recreate Stage-4 work from `main` or another branch; **followed** — Batch S4-02 built on the exact `claude/aicad-stage4-dev` HEAD it found (Batch S4-01's own commit).
5. authorize and begin AICAD-080 only then; **done** — Batches S4-00 (`AICAD-080`, `AICAD-081`), S4-01 (`AICAD-082`, `AICAD-083`, `AICAD-084`), and S4-02 (`AICAD-085`, `AICAD-086`, `AICAD-087`) are complete, see `project/reports/AICAD-080.md`..`087.md`.
6. do not begin AICAD-101+ / Stage 5 until Stage 4 later passes its owner hard gate. **Still in force** — only Batches S4-00, S4-01, and S4-02 (Stage 4) have run.

## Validation and evidence

Fresh AICAD-079C validation belongs in `project/reports/AICAD-079C.md`. `AICAD-080`/`081` validation belongs in `project/reports/AICAD-080.md`/`081.md`; `AICAD-082`/`083`/`084` validation belongs in `project/reports/AICAD-082.md`..`084.md`; `AICAD-085`/`086`/`087` validation belongs in `project/reports/AICAD-085.md`..`087.md`. Historical Stage-3 evidence remains historical and is not substituted for fresh results. This invocation had a real repository checkout (including a real native `cmake`/`cmake --build` pass against the installed OCCT 7.6.3 headers for `AICAD-086`'s own native additions) and ran `cargo fmt`/`clippy`/`test` directly (workspace-wide: 72/72 test binaries green, 1,148 total passing tests, 0 failures) plus the `scripts/ci/semantic_ref_harness.py` validate/self-test and `scripts/ci/stage4_task_audit.py --check` commands — see the task reports for exact output.
