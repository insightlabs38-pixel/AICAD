...
**The Stage-3 -> Stage-4 transition is complete and merged** (`claude/aicad-stage4-transition` -> `main`, PR #12, `f587251`). **Stage 4 implementation has started.** Batch S4-00 (`AICAD-080`, `AICAD-081`), Batch S4-01 (`AICAD-082`, `AICAD-083`, `AICAD-084` — geometry/topology/spatial predicate evaluation), Batch S4-02 (`AICAD-085`, `AICAD-086`, `AICAD-087` — explicit feature exports and lineage evidence), and Batch S4-03 (`AICAD-088`, `AICAD-089`, `AICAD-090` — the semantic-reference resolver, ambiguity diagnostic, and broken-reference diagnostic) are all done — see `project/reports/AICAD-080.md`..`090.md` and `project/CURRENT_STAGE.md`'s "Stage 4 — in progress" section. Batch S4-04 (`AICAD-091`, `AICAD-092`, `AICAD-093` — reference durability levels, geometry-fingerprint fallback under approved policy only, and raw topology handle epochs) is next.

**Working-branch discrepancy: resolved.** `origin/claude/aicad-stage4-dev` now exists (created from Batch S4-00's own working branch, preserving its `AICAD-080`/`081` commit rather than discarding it) and is the canonical Stage-4 branch; Batches S4-01, S4-02, and S4-03 were committed and pushed there directly. See `project/CURRENT_STAGE.md`'s "Working-branch note" for the full resolution. A future invocation should synchronize to `origin/claude/aicad-stage4-dev` with no further branch-naming ambiguity.
...
Authoritative fingerprint fallback (`AICAD-092` is next-batch, and remains restricted to evidence/ranking/benchmark use even once implemented, per D7/DL-8), raw-handle epochs, reference health reporting, Stage-4 source API, Stage-5 raw geometry, generalized feature tracing, interfaces, assemblies, configurations, external-asset infrastructure, and AICAD-101+ tasks remain unimplemented. `AICAD-082`..`084` added per-candidate predicate *evaluation* (given an already-identified candidate and, where needed, already-resolved evidence) — see `project/reports/AICAD-082.md`..`084.md`'s own "Limitations" sections for exactly which `TopologyPredicate`/`SpatialPredicate` variants remain unspecified pending a later task. `AICAD-085`..`087` added explicit-export registration and real per-operation lineage capture/classification (`unchanged`/`modified`/`split`/`deleted` for prior entities, `new`/`merged` for result entities) against a live build. `AICAD-088`..`090` added recipe/query *resolution* itself (`cad_query::resolve`: `Resolved`/`Ambiguous`/`Broken` from a real `Query`/`AnyRef`, including ranking-directive application and `FeatureLineage`/`Ancestry` resolution via query rewriting) plus the `REF-E102`/`REF-E101` diagnostics built from real outcomes — but, like `082`..`087`, every construction strategy without a corresponding query predicate (`ExplicitExport`/`StructuralRole`/`UserConfirmed`, and `SemanticQuery`'s own handle lookup) is only evaluator-contract-complete via an injected `ResolverContext` evidence hook, not yet wired to a real production evidence source, and nothing is yet wired to a real `FeatureGraph`/`ParametricBuildSession` build; see `project/reports/AICAD-085.md`..`090.md`'s own "Limitations" sections for the exact per-task boundary.
...
3. all sequential Stage-4 agents use the newest `origin/claude/aicad-stage4-dev` as canonical working state; **in force** — the branch exists and Batches S4-01/S4-02/S4-03 synchronized to it.
4. do not independently recreate Stage-4 work from `main` or another branch; **followed** — Batch S4-03 built on the exact `claude/aicad-stage4-dev` HEAD it found (Batch S4-02's own commit).
5. authorize and begin AICAD-080 only then; **done** — Batches S4-00 (`AICAD-080`, `AICAD-081`), S4-01 (`AICAD-082`, `AICAD-083`, `AICAD-084`), S4-02 (`AICAD-085`, `AICAD-086`, `AICAD-087`), and S4-03 (`AICAD-088`, `AICAD-089`, `AICAD-090`) are complete, see `project/reports/AICAD-080.md`..`090.md`.
6. do not begin AICAD-101+ / Stage 5 until Stage 4 later passes its owner hard gate. **Still in force** — only Batches S4-00 through S4-03 (Stage 4) have run.

## Validation and evidence

Fresh AICAD-079C validation belongs in `project/reports/AICAD-079C.md`. `AICAD-080`/`081` validation belongs in `project/reports/AICAD-080.md`/`081.md`; `AICAD-082`/`083`/`084` validation belongs in `project/reports/AICAD-082.md`..`084.md`; `AICAD-085`/`086`/`087` validation belongs in `project/reports/AICAD-085.md`..`087.md`; `AICAD-088`/`089`/`090` validation belongs in `project/reports/AICAD-088.md`..`090.md`. Historical Stage-3 evidence remains historical and is not substituted for fresh results. This invocation had a real repository checkout and ran `cargo fmt`/`clippy`/`test` directly after each task (workspace-wide, final Batch S4-03 state: 1,170 total passing tests, 0 failures) plus the `scripts/ci/semantic_ref_harness.py` validate/self-test and `scripts/ci/stage4_task_audit.py --check` commands after each task — see the task reports for exact per-task output.
