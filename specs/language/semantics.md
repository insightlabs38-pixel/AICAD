...
Functions are ordinary typed callables. User-defined structs/enums/functions may use generic type parameters under D17, with an optional nominal interface-conformance bound per parameter (`T: Interface1 + Interface2`) under D27/DL-29 (`AICAD-132`): `interface Name { field: Type, ... }` declares a required-field contract; `struct`/`part implements Interface, ...` declares explicit, statically verified conformance to it. See `specs/language/types.md`'s own "Interfaces/protocols and bounded generics" section for the full implemented shape and its explicit exclusions (no inheritance, dynamic dispatch, or trait objects). Enums support unit, tuple-payload, and record-payload variants, with corresponding pattern destructuring. `Result<T,E>` and `Optional<T>` are ordinary generic prelude enum types, not compiler-special semantic categories. Result propagation is explicit through ordinary control flow such as `match`; no `?`-style propagation syntax is current.
