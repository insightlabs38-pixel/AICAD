...
/// `04_pattern_count_change`'s own intended query target ("bolt-pattern
/// instance 0... by lineage" -- `case.md`'s own "Intended query target"
/// section) does not actually have real lineage evidence available:
/// `radial_pattern` is not one of the five ops `cad_query::feature_lineage`
/// classifies (`crates/cad-cli/src/reference_replay.rs`'s own closed
/// dispatch), so a `generated_by(bolt_pattern)`/`generated_by(body)` query
/// cannot distinguish instance 0 from any of the other pattern members --
/// every cylindrical hole wall in `body` is equally "generated by" the same
/// single `cut` operation. This is a real, honest evidence gap this task
/// does not paper over by inventing pattern-instance lineage tracking (an
/// unresolved architecture question, not this task's to decide).
///
/// This case's own `Reasoning` section already names the real alternative:
/// instance 0 sits at a **fixed** position (`(bolt_circle_radius, 0)`
/// relative to the pattern axis) "regardless of `count`... a language/
/// builtin-level guarantee". `nearest()` tracks that fixed position exactly
/// like case `07`'s own established "position-tracking across a renamed/
/// reordered producer" precedent (`AICAD-099`) -- the real production path
/// this case's own intended lineage-based reference is honestly expressible
/// through, not a substitute weaker than what was asked for: instance 0's
/// own real centroid *is* that fixed point (distance 0), so every other
/// pattern member -- genuinely farther away by construction, in both the
/// 5-hole baseline and the 6-hole perturbed build -- never ties with it.
#[test]
fn case04_pattern_count_change_bolt_instance_zero_tracked_by_position_real_production_path() {
    let ctx = OcctContext::new().expect("context creation should succeed");
    // `bolt_circle_radius = 18mm`, instance 0 unrotated at angle 0 (`y =
    // 0`); `z` is the hole wall's own approximate mid-height within the
    // `8mm`-thick plate (`case.md`'s own baseline.aicad: `plate_thickness =
    // 8mm`) -- exact to within `keep_nearest`'s own real center-of-mass
    // measurement, not a hand-picked exact value.
    let instance_zero_position = crate_point3(18.0, 0.0, 4.0);
    let query = Query::new(EntityKind::Face)
        .with_clause(QueryClause::Geometry(GeometryPredicate::Cylindrical))
        .with_clause(QueryClause::Ranking(cad_query::RankingDirective::Nearest(
            cad_query::SpatialTarget::Point(instance_zero_position),
        )))
        .with_cardinality(cad_query::CardinalityExpectation::Unique)
        .scoped_to(FeatureAnchor::named("body"));
    ...
    for variant in ["baseline", "perturbed"] {
        let s = session(
            &ctx,
            &format!("{BENCH}/public/04_pattern_count_change/{variant}.aicad"),
        );
        expect_resolved_one(s.resolve(&query).expect("resolution should not error"));
    }
}
