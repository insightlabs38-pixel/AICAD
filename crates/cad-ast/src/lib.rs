...
//! (declarations) added the `ty`/`item`/`stmt` modules. `AICAD-043`
//! (control-flow syntax) added the `pattern` module and extended `expr`/
//! `stmt` with `if`/`for`/`while`/`loop`/`match`/`return`/`break`/
//! `continue`. Each node type is added by the parser task that first
//! needs to produce it, per `AGENTS.md` "No speculative future work" —
//! see each module's own docs for exactly which grammar productions it
//! covers and which are left for a later task in this batch.
...
pub use expr::{Arg, BinaryOp, BlockExpr, ElseExpr, Expr, ExprKind, IfExpr, MatchExpr, UnaryOp};
pub use ident::Ident;
pub use item::{
...
pub use stmt::{
    AssignStmt, Block, BreakStmt, ContinueStmt, ElseBranch, ExprStmt, ForStmt, IfStmt, LetStmt,
    LoopStmt, MatchArm, MatchArmBody, MatchStmt, ReturnStmt, Stmt, VarStmt, WhileStmt,
};
pub use ty::Type;
